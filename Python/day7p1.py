...
with open("res/day7input.txt") as file:
    for card in [l.split() for l in file]:
        card_type = sorted([card[0].count(c) for c in hand_order], reverse=True)[:2]
        types[type_order.index(card_type)].append(card)
